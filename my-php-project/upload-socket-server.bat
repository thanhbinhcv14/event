...
echo [3/4] Managing PM2 process on VPS...
echo Attempting to restart existing process...
ssh root@152.42.246.239 "cd /root/socket-server && pm2 restart socket-server" 2>nul
if %errorlevel% neq 0 (
    echo Process not found. Starting new PM2 process...
    ssh root@152.42.246.239 "cd /root/socket-server && pm2 start socket-server.js --name socket-server"
    if %errorlevel% neq 0 (
        echo ERROR: PM2 start failed!
        echo Please SSH manually and check:
        echo - cd /root/socket-server
        echo - pm2 start socket-server.js --name socket-server
        echo - pm2 list
    ) else (
